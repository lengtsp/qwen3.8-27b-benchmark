...
For the next repair stage, **do not use fixed crop rectangles**. [`scripts/build_auto_layout_ocr_candidates.py`](scripts/build_auto_layout_ocr_candidates.py) turns PP-DocLayoutV3 detections into dynamic semantic-block, header/footer-line, and only-when-needed text-tile crops, with every derived coordinate and source hash recorded in a candidate manifest. `/root/llm-cache/typhoon-ocr1.5-2b` is installed as an independent Thai/English OCR reader for those candidates at its documented 1800-px image limit. The workflow is: orientation confidence gate → detected regions → Qwen full-page record + Typhoon crop evidence → accept only independent agreement or explicit image review; disagreements stay in the review queue. It never uses a hard-coded `bbox` and never lets Qwen silently patch its own transcript. The detailed commands and policy are in [`docs/ppdoclayoutv3-24-page-sweep.md`](docs/ppdoclayoutv3-24-page-sweep.md).

On the warm Typhoon service, 165 automatic candidates were generated from the 24-page layout manifest. Terra visually rechecked four known failure anchors against the source image: dynamic page-1 header, page-1 clause, page-3 role, and page-4 meeting fields all passed (**4/4 field-level strict matches**, 0.099–1.330 s/candidate). This is useful evidence that the independent-reader repair path works; it is **not** 100% page accuracy and does not authorize bulk replacement of the other 161 candidates. See the field table and exact safety gate in [`docs/ppdoclayoutv3-24-page-sweep.md`](docs/ppdoclayoutv3-24-page-sweep.md).

**Pure Typhoon full-page result:** 24 independent 300-DPI pages were sent directly to Typhoon at 1273 × 1800 px—no Qwen, PP-DocLayoutV3, crop, or PDF text layer. All responses finished normally, with a median **4.603 s/page** and **268.35 aggregate output tok/s**. Terra scored the same frozen 96 anchors used in the Qwen full-page sweep at **96/96 (100.0%)**, versus Qwen full 2000's 92/96. This is an anchor score, not whole-page/CER accuracy: Typhoon still changed a page-4 statistic `๑๕๕` → `๑๔๕`, flattened/mis-associated parts of the page-8 table, and wrote `IaaS` as `laaS` on page 10. Use Typhoon as the page OCR baseline, but retain the image and verify material numbers/tables with the automatic crop workflow. Full details and reproducible command are in [`docs/ppdoclayoutv3-24-page-sweep.md`](docs/ppdoclayoutv3-24-page-sweep.md).

**Native 300-DPI / >2400-px-width control:** Typhoon also received the unscaled 2481 × 3508 px pages directly. It scored **95/96** frozen anchors—worse than 1800 px because page 1 drops `การ` from the official title—and repaired page-8's low/medium/high table-row pairing, but it did **not** repair `๑๕๕` → `๑๔๕` or `IaaS` → `laaS`. The source-verified visual audit found **9 word/numeral/identifier error occurrences** and a structural ratio of **73/74 = 98.65%** (all 24 reading-order/heading/list units correct; 49/50 table regions usable). One page-24 table cell is assigned to the wrong column. On the only clean, non-table embedded-text reference subset (pages 4–7), native and 1800-px both measured **5.244% nCER (491/9,363 characters)**. Native costs 6.706 versus 4.603 median seconds/page, so use it for a table/critical-field retry rather than the default text-page profile. The complete error list, measurement limits, and commands are in [`docs/ppdoclayoutv3-24-page-sweep.md`](docs/ppdoclayoutv3-24-page-sweep.md).

#### PaddleOCR-VL 1.6 + PP-DocLayoutV3 + Typhoon: clean 24-page validation

All three newly requested models are now local under `/root/llm-cache`: Typhoon OCR 1.5 2B (4.0 GB), PaddleOCR-VL 1.6 (1.8 GB), and PP-DocLayoutV3 (126 MB). The test used a fresh, exact 24-page set rendered from the supplied PDF at 300 DPI (2,481 × 3,508 px/page); it did **not** mix old crop assets into the input. PP-DocLayoutV3 ran in the document-parser client for geometry; PaddleOCR-VL and Typhoon each ran as independent GPU vLLM services. No PDF embedded text was sent to either OCR model.

| Reader / evidence | Frozen material anchors (24 × 4) | Clean prose nCER, pages 4–7 only | Layout / table result | Decision |
| --- | ---: | ---: | --- | --- |
| **Typhoon full page, 1273 × 1800** | **96 / 96 = 100.0%** | **5.244%** (491 / 9,363 chars) | Text reader only; still inspect tables | **Primary transcript** |
| PaddleOCR-VL 1.6 + PP-DocLayoutV3 Markdown | 40 / 96 = 41.7% | 7.0704% (662 / 9,363 chars) | reading order/heading/list 22/24; table association 42/47; table-region recall 47/50 | Geometry and routing evidence only |
| Safe hybrid | no automatic merged score | n/a | Paddle boxes/regions guide review; Typhoon supplies text | **Recommended hybrid policy** |

The latest Typhoon pass completed 24 pages in 109.309 s, with 28,477 generated tokens and **260.52 aggregate output tok/s**; this is not comparable to Paddle's parser wall time because that pipeline did not expose a matching end-to-end token metric. Terra's visual audit found Typhoon errors outside the frozen anchors—page 4 `๑๕๕` → `๑๔๕`, page 8 table row/cell association, and page 10 `IaaS` → `laaS`. PaddleOCR-VL introduces too many text, numeral, and role-column mutations to merge its Markdown into the transcript automatically. Use PP-DocLayoutV3 for auto orientation/region/table routing, Typhoon full-page OCR for candidate text, and require image review for disagreements, numbers, legal dates, identifiers, and left/right table roles. The full per-page error list, methodology, commands, and safe merge rule are in [`docs/paddleocr-vl-typhoon-hybrid-24p.md`](docs/paddleocr-vl-typhoon-hybrid-24p.md).

Within the **Qwen** experiments, the correct workflow is **full-page 2000** as the OCR record, followed by a targeted crop only for legally material dates, gazette/section numbers, locations, statistics, or a table row/cell. Automatic content cropping improves speed and local legibility but cannot replace the official header/title or reconstruct geometry-faithful table cells. The exact per-page scores, error types, 2600 control, crop policy, installation, and reproducible commands are in [`docs/ppdoclayoutv3-24-page-sweep.md`](docs/ppdoclayoutv3-24-page-sweep.md). PP-DocLayoutV3 is installed locally at `/root/llm-cache/PP-DocLayoutV3` and runs CPU-only in `/root/venvs/ppstructurev3`, independently of the vLLM GPU service.
